...
TEST(ToUnderlyingTest, ConvertsEnumCorrectly) {
  ...
  enum class Mood : int { Happy = 1, Sad = -1 };
...
TEST(ToUnderlyingTest, HandlesMinMaxValues) {
  ...
  EXPECT_EQ(stdcpp::to_underlying(Numbers::Min),
            std::numeric_limits<int>::min());
  EXPECT_EQ(stdcpp::to_underlying(Numbers::Max),
...
TEST(SSizeTest, ReturnsCorrectSignedSize) {
  ...
  static_assert(std::is_signed<decltype(vec_size)>::value,
                "ssize should return a signed size for vector");
  static_assert(std::is_signed<decltype(str_size)>::value,
                "ssize should return a signed size for string");
...
TEST(AddConstTest, AddsConstToType) {
  static_assert(std::is_same<stdcpp::add_const<int>::type, const int>::value,
                "add_const should add const to type");
  static_assert(
      std::is_same<stdcpp::add_const<const int>::type, const int>::value,
      "add_const should not add const to const type");
  static_assert(std::is_same<stdcpp::add_const<int&>::type, int&>::value,
                "add_const should not add const to reference type");
  static_assert(
      std::is_same<stdcpp::add_const<const int&>::type, const int&>::value,
      "add_const should add const to const reference type");
}

TEST(AsConstTest, AddsConstToLValue) {
  int i = 10;
  const auto& const_i = stdcpp::as_const(i);
  static_assert(std::is_same<decltype(const_i), const int&>::value,
                "as_const should add const to lvalue");
}
